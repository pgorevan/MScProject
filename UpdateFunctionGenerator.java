...
public class UpdateFunctionGenerator {
	...
	/**
	 * Generates a random boolean expression in binary tree form from a set of
	 * genes
	 * 
	 * @param noPossActivators
	 *            The number of possible activator genes the function can use
	 * @param noPossSupressors
	 *            The number of possible suppressor genes the function can use
	 * @param geneVariables
	 *            The number of possible suppressor genes the function can use
	 * @return updateFunction Binary tree representing a boolean function of the
	 *         form suppressors AND NOT (activators)
	 */
	public static ExpressionTree createUpdateFunction(int noPossActivators, int noPossSupressors,
			Set<Gene> geneVariables) {

		ExpressionTree updateFunction = new ExpressionTree(); // final update
																// function
		ExpressionTree activatorTree = new ExpressionTree(); // activator half
																// of the
																// function
		ExpressionTree suppressorTree = new ExpressionTree(); // suppressor half
																// of the
																// function

		if (!geneVariables.isEmpty()) {
			// Create a array representing the preorder traversal of the
			// activator subtree
		...
		// If there activators and no suppressors update function is the
		// activator tree
		if (!activatorTree.isEmpty() && suppressorTree.isEmpty()) {
			updateFunction.root = activatorTree.root;
		}
		// If there are suppressors present but no activators update function
		// consists of the repressor tree
		if (activatorTree.isEmpty() && !suppressorTree.isEmpty()) {
			NotNode not = new NotNode(suppressorTree.root);
			suppressorTree.root = not;
			updateFunction.root = suppressorTree.root;
		}
		// If there are both activators and suppressors join the subtrees
		// together to form the update function
		if (!activatorTree.isEmpty() && !suppressorTree.isEmpty()) {
			NotNode not = new NotNode(suppressorTree.root);
	 ...
	 * 
	 * @param noPossVars
	 *            Number of variables ie leaf nodes of the tree
	 * @param GeneVariables
	 *            The set of genes
	 * @return arrayRepOfTree An int array containing the preorder traversal of
	 *         a binary tree
	 */
	 ...
	 * 
	 * @param nodes
	 *            array of 1 and 0s representing internal nodes(1s) and
	 *            leafs(0s)
	 * 
	...
	/**
	 * Converts a array representing the shape of a binary tree
	 * to one populated with a random selection of expressionNode in the same shape
	 * 
	 * @param a  int[] An array of 1s and 0s representing leaves and non-leaves of a tree
	 * 
	 * @return udfArray An array containing the preorder traversal of a expression tree
	 */
